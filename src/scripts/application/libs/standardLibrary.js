define(['eventmanager'], function (eventmanager) {
    ...
    function worldPosToGridPos(config) {
        ...
        var coords = isoToTwoD(config);
        coords.x = coords.x/(app.config.actor.car.width/2);
        coords.y = coords.y/(app.config.actor.car.height/2);
    ...
    function carWorldPosToIsoWorldPos(config){
        config = _.cloneDeep(config);
        // Add correction for the centering of the map
        var x = config.y + config.x;
        var y = config.y - config.x;
